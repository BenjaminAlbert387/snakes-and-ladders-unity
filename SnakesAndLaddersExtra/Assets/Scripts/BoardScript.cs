...
public class BoardScript
{
  ...
  {
    // Create a new dictonary for each player. Contains both the player and their position
    playerPosition = new Dictionary<Player, int>();

    // Create 100 grid squares, used for counter movement
    ...
    /* As the player counter moves, the number of grid
  ...
  // Metho that updates the board based on the user dice roll
  public List<int> UpdateBoard(Player player, int DiceRoll)
  {
    // Creates a list. Stored in the variable 
    List<int> result = new List<int>();

    /* For loop that updates the player counter position
    ...
    // If loop that makes the indexing consistent
